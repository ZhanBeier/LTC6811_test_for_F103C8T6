...
int main(void) {
    ...
    while (1) {
        ...
        if (pec_error == 255) {
            snprintf(output_buf, 20, "PEC ERROR!\n\r");
        } else {
            snprintf(output_buf, 80, "CELL1 %.4fv\nCELL2 %.4fv\nCELL3 %.4fv\nCELL7 %.4fv\nCELL8 %.4fv\n-----\n\r",
                     (float) cell_voltage[0][0] / 10000, (float) cell_voltage[0][1] / 10000,
                     (float) cell_voltage[0][2] / 10000, (float) cell_voltage[0][6] / 10000,
                     (float) cell_voltage[0][7] / 10000);
        }

        HAL_UART_Transmit(&huart1, (uint8_t *) output_buf, sizeof(output_buf), 10);
